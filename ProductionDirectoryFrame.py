...
#----------------------------------------------------------------------
'''
  1.05  2016/12/13  SPN     Updated OnPE075Button, OnPE038Button methods: removed verification meter check
                            since it has been added to the calibration GUI
  1.04  2015/01/19  SPN     -Updated OnPE075Button, OnPE038Button methods:  set default meter to PE102
                            for older rev
  1.03  2015/01/13  SPN     Updated OnPE075Button, OnPE038Button methods: Change default meter to PE202
...
class Frame(DirectoryFrame.Frame):
    def __init__(self, parent, main):
        ...
        self.DirectoryLabel = wx.StaticText(id=DIRECTORY_TEXT_ID,label='CALIBRATION', parent=self.panel, size=wx.Size(150, 25), style=wx.ALIGN_CENTER|wx.ST_NO_AUTORESIZE)
    ...
    def OnPE075Button(self, event):        
        #----------------------------------
        # Run System Test (once per day max frequency)
        #----------------------------------        
        try:
        ...
        if is_test:
            myUtil.MsgDialog(self,'Please perform the daily system test.')
        ...
        if x not in myFlowMeters.FLOW_METER_OBJECT_DICT.keys():
            # if no response then try rev J as default
            mut = myFlowMeters.PE102_REV_J()
        ...
        if mut.revision == myFlowMeters.PE102_REV_J().revision:
            # default to PE202 standard for current rev
            mut.SetIdentity075_PE202()
        else:
...
##            return
        ...
        self.page = self.DestroyOpenPage()
        self.page = CalibrationFrame.Frame(self,
                                            mut,
                                            self.ref1,
                                            self.ref2,
                                            HF_SYSTEM)
    ...
    def OnPE038Button(self, event):
        ...
        #----------------------------------        
        try:
            d = myFile.myFile(LAST_LF_TEST_DATE_FILE).GetTextByLines()
            if len(d) == 0:
                is_test = True
            elif myDateTime.IsStringDate(d[0]):            
                dt = datetime.datetime.today() - myDateTime.GetDateFromString(d[0])
