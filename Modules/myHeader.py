...
#----------------------------------------------------------------------
'''
 1.23   18/03/28    SPN     -Zeroed REF3_POLY_COEF to disable curve fit for recalibrated ref meters
 1.22   18/02/19    SPN     -Updated REF1_POLY_COEF, REF2_POLY_COEF constants while HF ref meters are out for recal
 1.21   16/12/29    SPN     -Changed DEFAULT_PROFILE to DEFAULT_PROFILE_PATH and addedd DEFAULT_PROFILE_FOLDER
                            -Recal FS verify meter and updated ADC constants
 1.20   16/12/12    SPN     -Added REFi_POLY_COEF constants
 1.19   16/12/8     SPN     -Updated REF2_MAX_Q constant from 0.95 to 1.1 to support HF transition
                             flow rate from 0.9 to 1.09
 1.18   16/10/14    SPN     -Added MSG_TEST_STOPPED constant
                            -Updated VERIFICATION ADC constants to implement new verification meters
                            with the new electrode design
 1.17   16/05/06    SPN     -Updated OPC_SERVER_NAME to reference KepDirect v5
 1.16   15/05/04    SPN     -Reassigned PLC_RELAY_C34 to PLC_RELAY_C16
                            -Added PLC_RELAY_C17 (Unlock Clamp)
 1.15   15/05/01    SPN     -Added PLC_RELAY_C34 (Lock Clamp)
 1.14   15/03/02    SPN     -Updated HF_VERIFICATION ADC constants by reaming & recalibrating/designating
                            new PE202 verification meter
 1.13   15/02/27    SPN     -Updated HF_VERIFICATION ADC constants by calibrating/designating
                            new PE202 verification meter
 1.12   15/02/20    SPN     -Updated LF_VERIFICATION ADC constants by calibrating/designating
                            new PE202 verification meter
...
REF1_K                      = 2000.0    # HF FS (upstream) Siemens
REF2_K                      = 50000.0   # HF LS (downstream) Siemens
REF3_K                      = 10000.0   # LF FS (upstream) Siemens
REF4_K                      = 250000.0  # LF LS (downstream) Coriolis
CHECK_STANDARD_K            = 20000.0
# reference meter corrections:
# % error(Q) = AQ^(n) + BQ^(n-1) + CQ
# REFi_POLY_COEF = [A B C]
##REF1_POLY_COEF = [-5.930288995568207e-07, 3.8618199374664085e-05, -0.0010081098797023555, 0.014203078865982908, -0.12582919514671012, 0.707395645237522, -1.7112202842065016]
##REF2_POLY_COEF = [-53.533777152804404, 173.2592154555477, -218.9483814080259, 134.17316176928972, -38.99962233132781, 4.0261176608679605]
##REF3_POLY_COEF = [-0.14757232420323166, 1.479704237026098, -5.617941147613952, 9.972349563819874, -8.240236646305766, 2.602606657488698, 0.008822082538511314]
REF1_POLY_COEF = [0]
REF2_POLY_COEF = [0]
REF3_POLY_COEF = [0]
REF4_POLY_COEF = [0]
##REF3_CORRECTION     = [0.414585, 0.170638, 0.00101516]  # Old coefficients Jim H generated
...
AI_UPDATE_RATE              = 500
OPC_SERVER_NAME             = 'AutomationDirect.KEPDirect.V5'
...
PLC_RELAY_C16               = 'PE CAL SYS.PE CAL SYS.RELAY C16'
PLC_RELAY_C17               = 'PE CAL SYS.PE CAL SYS.RELAY C17'
...
# Calibrated LF reference meter on 01/19/2015:
# fsadc:  286.817252958, zradc: 0.37088613726
# Calibrated HF reference meter on 01/19/2015:
# fsadc: 422.388698684, zradc: 0.82015973539
# Calibrated PE202 LF reference meter on 02/20/2015:
# fsadc:  339.442, zradc: 2.10079
# Calibrated PE202 HF reference meter on 02/23/2015:
# fsadc:  399.741, zradc: 3.63891
# Reamed & Recalibrated PE202 HF reference meter on 03/02/2015:
# fsadc:  398.055, zradc: 1.01284
# Calibrated new PE202 HF verify meter (new electrode design) on 10/14/2016:
# fsadc: 424.116, zradc: 1.03133
# Calibrated new PE202 LF verify meter (new electrode design) on 10/14/2016:
# fsadc: 330.874, zradc: 0.20069
# Recalibrated PE202 HF verify meter (new electrode design) on 12/29/2016:
# fsadc: 428.894, zradc: 0.72122
HF_VERIFICATION_FSADC = '428.894'
HF_VERIFICATION_ZRADC = '0.72122'
LF_VERIFICATION_FSADC = '330.874'
LF_VERIFICATION_ZRADC = '0.20069'
#----------------------------------
# Custom Multipoint
#----------------------------------
DEFAULT_PROFILE_FOLDER = SYSTEM_PATH + '\\Profiles'
DEFAULT_PROFILE_PATH = DEFAULT_PROFILE_FOLDER + '\\DEFAULT PROFILE.txt'
#----------------------------------
# Messages
#----------------------------------
MSG_TEST_STOPPED = 'ERROR: Safety check. Test aborted.\n\n\
Possible causes:\n\
-Resevoir level low\n\
