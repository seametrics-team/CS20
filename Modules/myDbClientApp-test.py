...
##    import myDbClientApp as myDbClientApp
    ...
    data[TEST_RESULT_BENCH_ID_KEY] = 9
    
    client = PostCalibrationDataClient(data)
    client.start()

def Test3():
    client = GetCalibrationDataClient('00000002', None)
    client.start()
...
##    client = GetProductDataClient('02131856', q)
    client = GetProductDataClient('092013001746', q)
    client.start()
    print 'client finished::'
    q_empty = q.empty()
    print 'q empty? ', q_empty
    item = q.get()
    print '---meter data---'
